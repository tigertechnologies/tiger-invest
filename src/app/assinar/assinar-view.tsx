  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      if ('status' in s && s.status === 'paid') {
        ...
        setTimeout(() => router.push('/invest'), 1600)
      }
    }, 4000)
            ...
            {!ok && (
              <form onSubmit={submit}>
                ...
                <label className="as-accept">
                  ...
                  <span>Li e aceito os <Link href="/termos" target="_blank">Termos de Uso e a Política de Privacidade (LGPD)</Link>. Declaro estar ciente de que o Tiger Labs é uma plataforma de tecnologia, <strong>não é recomendação de investimento</strong> e não se responsabiliza por decisões ou perdas financeiras.</span>
                </label>

                <button className="t-btn" style={{ marginTop: 18 }} disabled={loading || !accepted}>{loading ? '...' : 'Criar conta e ir para o pagamento'}</button>
          ...
          {step === 'pix' && (<>
            ...
            {!paid && pix && !pix.ok && (
              <div style={{ marginTop: 8 }}>
                ...
                <button className="t-btn" onClick={gerarPix} style={{ marginTop: 8 }}>Tentar novamente</button>
              </div>
            )}
          </>)}
