...
import { loadAdminData, type AdminData } from '@/lib/invest/admin-data'
import { isAdminUser } from '@/lib/admin-check'

/** Garante que quem chama é admin (ADMIN_EMAILS ou profiles.role='admin'). Lança se não for. */
async function requireAdmin(): Promise<void> {
  const supabase = createClient()
  const { data } = await supabase.auth.getUser()
  if (!(await isAdminUser(data.user))) throw new Error('Acesso negado.')
}
