...
} from '@/lib/invest/perps'
import { emissionFor, emissionTone, dilutionAdjusted } from '@/lib/invest/inflation'
import { pickTopRisk, type TopRisk } from '@/lib/invest/health'
...
export default function DashboardApp({
  ...
  // dados on-chain por pool: status real de faixa (ticks), preço USD do token de cotação e quantidades atuais
  const [poolChain, setPoolChain] = useState<Record<string, { inRange: boolean; quoteUsd: number; base: number | null; quote: number | null }>>({})
  // monta o objeto acima a partir da resposta de /api/lp-position e do ratio escolhido
  const chainInfo = (d: any, chosen: number) => {
    const t1PerT0 = chosen === d.ratio_t1_per_t0
    return {
      inRange: !!d.in_range,
      quoteUsd: (t1PerT0 ? d.price1_usd : d.price0_usd) || 1,
      base: t1PerT0 ? d.amount0 : d.amount1,
      quote: t1PerT0 ? d.amount1 : d.amount0,
    }
  }
  ...
  const alerts = useMemo(() => {
    ...
    for (const p of pools) {
      const ch = p.id ? poolChain[p.id] : undefined
      ...
      if (ch ? !ch.inRange : (price < p.low_range || price > p.high_range)) out.push({ id: 'plo' + p.id, tone: 'warn', icon: '⚠', title: `Pool ${p.par1}/${p.par2} FORA do range`, text: `${p.par1} em ${fmt(price)} · deixou de gerar taxas` })
    ...
    return out
  }, [priced, levels, txs, pools, live, poolRatio, poolChain])
  ...
  const cats = useMemo(() => {
    ...
    return [
      { n: 'Ethereum', v: bs('ETH'), c: '#19D3A2' }, { n: 'Bitcoin', v: bs('BTC'), c: '#00FF88' },
      { n: 'Solana', v: bs('SOL'), c: '#40BEFF' }, { n: 'Altcoins', v: other, c: '#B6FF5C' },
      { n: 'Ações', v: priced.filter(h => h.kind === 'stock').reduce((a, h) => a + valOf(h), 0), c: '#40BEFF' },
      { n: 'Caixa', v: priced.filter(h => h.kind === 'cash').reduce((a, h) => a + valOf(h), 0), c: '#7DDFFF' },
  ...
  async function saveBuy() {
    ...
    const payload = { user_id: userId, symbol: f.symbol.toUpperCase(), name: f.name || f.symbol, cg_id: f.cg_id, color: f.color || '#19D3A2', rede: f.rede, corretora: f.corretora, carteira: f.carteira, buy_date: f.buy_date, qty: num(f.qty), buy_price: num(f.buy_price), stop_limit: num(f.stop_limit), target: num(f.target), meta_pct: num(f.meta_pct) }
  ...
  async function saveMove() {
    ...
    const payload: any = {
      user_id: userId, symbol: f.symbol.toUpperCase(), name: f.name, cg_id: f.cg_id, color: f.color || '#19D3A2',
  ...
  const pickMetaCoin = (c: any) => {
    ...
    setMetaForm((prev: any) => ({
      ...
      color: existing?.color || '#19D3A2', target: !existing,
  ...
  async function saveMeta() {
    ...
    if (f.id) {
      ...
      await supabase.from('holdings').insert({
        ...
        cg_id: f.cg_id, color: f.color || '#19D3A2', qty: 0, price: 0, invested: 0, current_value: null, meta_pct: v, sort: 50,
      })
  ...
  const distrib = [
    { n: 'Cripto', v: cryptoVal, c: '#00FF88' },
    { n: 'Ações/ETFs', v: stockVal, c: '#40BEFF' },
    { n: 'Caixa', v: t.cashVal, c: '#7DDFFF' },
    { n: 'Pools', v: poolsVal, c: '#2BFFC6' },
  ].filter(x => x.v > 0)
  ...
  const openBuy = (h: Holding | null) => { setCoinQuery(''); setCoinResults(null); setCoinManual(false); setHistPrice(null); setTxForm(h
    ...
    : { symbol: '', name: '', cg_id: '', color: '#19D3A2', meta_pct: '', rede: '', corretora: '', carteira: '', buy_date: new Date().toISOString().slice(0, 10), qty: '', buy_price: '', stop_limit: '', target: '', isNew: true }) }
  ...
  async function syncPosition(p: Pool) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        const r = await fetch(`/api/lp-position?network=${p.network || 'base'}&id=${p.position_id}`)
        d = await r.json()
        if (d?.ok) break
      ...
      if (chosen && p.id) { setPoolRatio(prev => ({ ...prev, [p.id!]: chosen })); setPoolChain(prev => ({ ...prev, [p.id!]: chainInfo(d, chosen) })) }
      await refetch()
  ...
  useEffect(() => {
    pools.forEach(p => {
      if (p.position_id && p.id && !poolRatio[p.id]) {
        fetch(`/api/lp-position?network=${p.network || 'base'}&id=${p.position_id}`).then(r => r.json()).then(async d => {
          if (!d?.ok) return
          ...
          if (chosen) { setPoolRatio(prev => ({ ...prev, [p.id!]: chosen })); setPoolChain(prev => ({ ...prev, [p.id!]: chainInfo(d, chosen) })) }
  ...
  return (
    <>
      ...
      <div className="app">
        <div className="top">
          <LogoMark size={38} />
          <div className="brand"><b>Minha Carteira</b><span>Tiger Labs · Invest</span></div>
          <div className="top-actions">
            ...
            {isAdmin && <button className="logout" style={{ borderColor: 'rgba(255,176,32,.5)', color: '#FFB020' }} onClick={() => router.push('/admin/assinaturas')}>Admin</button>}
        ...
        <div className="scroll">
          ...
          <section className={`screen ${tab === 'inicio' ? 'active' : ''}`}>
            ...
            {(() => {
              ...
              return (
                <div className="t-card section-gap resumo-card">
            ...
            <div className="t-card section-gap"><div className="eyebrow">Alocação atual</div>
          ...
          <section className={`screen ${tab === 'carteira' ? 'active' : ''}`}>
            ...
            <div className="t-card section-gap">{holdings.filter(h => h.kind === 'cash').map(h => (<div className="kv" key={h.id} onClick={() => { setEditDraft({ ...h }); setCashInput(String(h.current_value ?? 0).replace('.', ',')) }} style={{ cursor: 'pointer' }}><span className="k">{h.name}</span><span className="v num">{usd(h.current_value ?? 0)}</span></div>))}</div>
          </section>
          ...
          <section className={`screen ${tab === 'cotacao' ? 'active' : ''}`}>
            ...
            {top50 && top50.slice().sort((a: any, b: any) => (b.mcap || 0) - (a.mcap || 0)).map((c: any, i: number) => (
              <div className="qrow" key={c.id}>
                <div className="qsym" style={{ background: '#0f1a14' }}>{c.img ? <img src={c.img} alt="" /> : c.symbol.slice(0, 3)}</div>
                <div className="qname"><b><span style={{ color: 'var(--faint)', fontFamily: "'JetBrains Mono', ui-monospace, monospace", fontSize: 12 }}>{i + 1}. </span>{c.name}</b><span>{c.symbol} · MC {abbrMC(c.mcap || 0)}</span></div>
          ...
          <section className={`screen ${tab === 'pools' ? 'active' : ''}`}>
            ...
            {pools.map(p => {
              const price = (p.id && poolRatio[p.id]) ? poolRatio[p.id] : (live[p.par1_cg_id]?.usd ?? 0)
              const ch = p.id ? poolChain[p.id] : undefined
              // com NFT sincronizado, o status vem dos ticks on-chain (não depende da unidade cadastrada)
              const below = ch ? (!ch.inRange && price < p.low_range) : (price > 0 && p.low_range > 0 && price < p.low_range)
              const above = ch ? (!ch.inRange && !below) : (price > 0 && p.high_range > 0 && price > p.high_range)
              const inRange = ch ? ch.inRange : (price > 0 && !below && !above && p.low_range > 0)
              const quoteUsd = ch?.quoteUsd ?? 1   // 1 = par 2 é stablecoin (comportamento antigo)
              const span = p.high_range - p.low_range
              ...
              return (<div className="poolcard" key={p.id}>
                ...
                <PoolChart par1={p.par1} par2={p.par2} cgId={p.par1_cg_id} poolId={p.id} price={price} low={p.low_range} high={p.high_range} currentValue={p.current_value} aporte={p.aporte} entryPrice={p.entry_price} quoteUsd={quoteUsd} nowAmounts={ch && ch.base != null && ch.quote != null ? { base: ch.base, quote: ch.quote } : undefined} />
                ...
                <div style={{ marginTop: 12 }}>
                  ...
                  {il && (<><div className="kv"><span className="k">{p.par1} vs. entrada ({quoteUsd !== 1 ? `${fmt(p.entry_price!, 5)} ${p.par2}` : usd(p.entry_price!)})</span><span className={`v num ${il.chg >= 0 ? 'up' : 'down'}`}>{pct(il.chg)}</span></div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="t-card" style={{ marginTop: 12, background: 'rgba(6,14,10,.5)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span className="k" style={{ fontSize: 13 }}>Taxa já cobriu a divergência?</span>
                        <b style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontWeight: 800, color: cor }}>{cobriu ? '✓ SIM' : '✗ AINDA NÃO'}</b>
                      </div>
                ...
                <div className="grid2" style={{ marginTop: 12 }}><button className="t-btn ghost" onClick={() => openPool(p)}>Editar</button>{p.link ? <a className="t-btn ghost" style={{ textDecoration: 'none', textAlign: 'center', lineHeight: '1.6' }} href={p.link} target="_blank" rel="noreferrer">Abrir dApp</a> : null}</div>{p.position_id ? <button className="t-btn ghost" style={{ marginTop: 8, width: '100%' }} disabled={syncing === p.id} onClick={() => syncPosition(p)}>{syncing === p.id ? 'Sincronizando…' : '🔄 Sincronizar taxas'}</button> : null}
              </div>)
            })}
            ...
            {!has(2) ? (
              <div className="lock-card">
                ...
                <button className="t-btn" style={{ maxWidth: 240, margin: '0 auto' }} onClick={() => setUpgrade({ tier: 2, feature: 'Onde abrir pool' })}>Liberar no TIGER PRO →</button>
              </div>
            ) : (() => {
                  ...
                  <div className={`poolcard ${it.highlight ? 'hot' : ''}`} key={i} style={{ cursor: 'pointer' }} onClick={() => setExpandedIdea(open ? null : keyOf(it))}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontWeight: 700, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{it.name}</div>
                        <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>{it.dex} · {it.network}</div>
                      </div>
                      <div style={{ textAlign: 'right', flex: 'none' }}>
                        <div style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontWeight: 800, fontSize: 19, color: netColor, lineHeight: 1 }}>{netStr}</div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
                      {it.yieldGrade && <span style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontWeight: 800, fontSize: 10, color: '#04120b', background: gc, padding: '2px 8px', borderRadius: 999 }}>{it.yieldGrade} {it.yieldScore}</span>}
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 700, fontFamily: "'JetBrains Mono', ui-monospace, monospace", color: stColor }}><span style={{ width: 7, height: 7, borderRadius: 999, background: stColor, boxShadow: `0 0 7px ${stColor}` }} />{it.verdictLabel}</span>
                      <span style={{ fontSize: 10.5, color: ilColor, fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>IL {it.il}</span>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8, fontSize: 11, color: 'var(--muted)', fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>
                    ...
                    {open && (
                      <div style={{ marginTop: 14, borderTop: '1px solid var(--line)', paddingTop: 14 }} onClick={stop}>
                        ...
                        <div className="grid2" style={{ marginTop: 12 }}>
                          <a className="t-btn ghost" style={{ textDecoration: 'none', textAlign: 'center', lineHeight: '1.4' }} href={it.dexUrl || dataLink} target="_blank" rel="noreferrer" onClick={stop}>Abrir na {it.dex} ↗</a>
                          <button className="t-btn ghost" onClick={(e) => { stop(e); has(3) ? openCalc(it) : setUpgrade({ tier: 3, feature: 'Calculadora de IL' }) }}>🧮 Simular {!has(3) && '🔒'}</button>
                        </div>
                        {dataLink && <a className="t-btn ghost" style={{ textDecoration: 'none', textAlign: 'center', lineHeight: '1.6', marginTop: 8, fontSize: 12, opacity: .82 }} href={dataLink} target="_blank" rel="noreferrer" onClick={stop}>Ver dados e histórico ↗</a>}
                      </div>
                    )}
                  </div>
                )
          ...
          <section className={`screen ${tab === 'perps' ? 'active' : ''}`}>
            {(() => {
              ...
              return (<>
                ...
                <button className="t-btn" onClick={() => openPerpForm()}>+ Abrir posição long / short</button>
                    ...
                    return (
                      <div className="poolcard perp-card" key={p.id} style={{ marginBottom: 12 }}>
                        ...
                        <div style={{ marginTop: 10 }}>
                          ...
                          <div className="kv"><span className="k">Mark {m ? <span className={`t-chip ${(m.chg24 || 0) >= 0 ? 'up' : 'down'}`}>{pct(m.chg24 || 0)}</span> : null}</span><span className="v num">{mark > 0 ? usd(mark) : '—'}</span></div>
                          ...
                          {(() => {
                            ...
                            return (<>
                              <div className="kv"><span className="k">🎯 Alvo / TP</span><span className="v num">{p.tp ? <>{usd(p.tp)} {tpHit ? <span className="t-chip up">atingido ✓</span> : tpNear != null && tpNear < 3 ? <span className="t-chip up">{fmt(tpNear, 1)}%</span> : null}</> : <span style={{ color: 'var(--faint)' }}>—</span>}</span></div>
                              <div className="kv"><span className="k">🛑 Stop / SL</span><span className="v num">{p.sl ? <>{usd(p.sl)} {slHit ? <span className="t-chip down">rompido ⚠</span> : slNear != null && slNear < 3 ? <span className="t-chip down">{fmt(slNear, 1)}%</span> : null}</> : <span style={{ color: 'var(--faint)' }}>—</span>}</span></div>
                            </>)
                          })()}
                        </div>
                        {dist != null && (
                        ...
                        <div className="grid2" style={{ marginTop: 14 }}>
                          <a className="t-btn ghost" style={{ textDecoration: 'none', textAlign: 'center', lineHeight: '1.4' }} href="https://app.ondoperps.xyz/" target="_blank" rel="noreferrer">Gerenciar na Ondo ↗</a>
                          <button className="t-btn ghost" onClick={() => setPerpClose({ id: p.id, symbol: p.symbol, side: p.side, size: p.size, entry_price: p.entry_price, leverage: p.leverage, price: mark > 0 ? String(mark) : '' })}>Encerrar</button>
                        </div>
                ...
                {/* Histórico encerrado */}
                {closedPos.length > 0 && (
                  <div className="t-card section-gap">
          ...
          <section className={`screen ${tab === 'aportes' ? 'active' : ''}`}>
            ...
            <div className="t-card section-gap"><div className="eyebrow" style={{ marginBottom: 6 }}>Onde está o capital</div>
            ...
            <div className="t-card section-gap"><div className="eyebrow" style={{ marginBottom: 8 }}>{flows.length ? 'Tempo decorrido (desde o 1º aporte)' : 'Tempo médio das posições'}</div>
              {flows.length
            ...
            <div className="t-card section-gap"><div className="eyebrow" style={{ marginBottom: 6 }}>Resultado por período</div>
            ...
            {flows.length > 0 && <div className="t-card section-gap"><div className="eyebrow" style={{ marginBottom: 4 }}>Extrato · toque p/ editar a data e o valor</div>
          ...
          <section className={`screen ${tab === 'metas' ? 'active' : ''}`}>
            ...
            <div className="t-card" style={{ background: 'linear-gradient(180deg,rgba(0,255,136,.08),rgba(8,20,14,.5))', border: '1px solid rgba(0,255,136,.22)' }}>
              <div style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontWeight: 700, fontSize: 14, marginBottom: 6 }}>Como funciona?</div>
                      ...
                      {(() => {
                        ...
                        return (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 7 }}>
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 10.5, fontWeight: 700, fontFamily: "'JetBrains Mono', ui-monospace, monospace", color: c, background: `${c}1a`, border: `1px solid ${c}55`, padding: '3px 9px', borderRadius: 999 }}><span style={{ width: 6, height: 6, borderRadius: 999, background: c, boxShadow: `0 0 7px ${c}` }} />{label}</span>
                ...
                return (
                  <div className="t-card">
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 8 }}>
                      <b style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontSize: 26, color: tone, textTransform: 'uppercase', letterSpacing: 1 }}>{pulse.regime}</b>
          ...
          <section className={`screen ${tab === 'tiger100' ? 'active' : ''}`}>
            ...
            <div className="t-card" style={{ background: 'linear-gradient(180deg,rgba(64,190,255,.10),rgba(8,20,14,.5))', border: '1px solid rgba(64,190,255,.25)' }}>
              <div style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontWeight: 700, fontSize: 14, marginBottom: 6 }}>O que é o Tiger 100?</div>
              <p className="foot-note" style={{ textAlign: 'left', padding: 0, lineHeight: 1.55 }}>
                ...
                <div className="t-card">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                    <div>
                      ...
                      <div style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontWeight: 800, fontSize: 34, lineHeight: 1 }}>{x.level != null ? x.level.toLocaleString('pt-BR', { maximumFractionDigits: 0 }) : '—'}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontWeight: 800, fontSize: 20, color: (x.ret24 || 0) >= 0 ? 'var(--green)' : 'var(--red)' }}>{x.ret24 != null ? pct(x.ret24) : '—'}</div>
                ...
                <div className="t-card section-gap">
                  ...
                  {x.composition.map((c: any) => (
                    <div key={c.id} style={{ marginBottom: 9 }}>
                      ...
                      <div style={{ height: 5, borderRadius: 999, background: 'rgba(255,255,255,.07)', overflow: 'hidden' }}><div style={{ height: '100%', width: `${Math.min(100, c.weight / x.composition[0].weight * 100)}%`, background: 'linear-gradient(90deg,#40BEFF,#2BFFC6)' }} /></div>
                    </div>
                  ))}
                </div>
          ...
          <p className="foot-note"><b style={{color:'var(--pink-bright)',fontFamily: "'Space Grotesk', Inter, sans-serif"}}>Tiger Labs</b> · Não é recomendação de investimento. Todo e qualquer investimento é por conta e risco do usuário — estude os ativos antes de aplicar seu capital.</p>
        </div>
            ...
            <div className="sheet"><div className="grabber" /><div className="sheet-scroll">
              ...
              {metaForm.isNew && (<>
                ...
                {metaCoinResults && metaCoinResults.length > 0 && (
                  <div className="t-card" style={{ marginTop: 8, padding: 6 }}>
                    {metaCoinResults.map((c: any) => (
                      <div key={c.id} onClick={() => pickMetaCoin(c)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px', cursor: 'pointer', borderRadius: 8 }}>
                        <div className="qsym" style={{ width: 28, height: 28, background: '#0f1a14' }}>{c.image ? <img src={c.image} alt="" /> : (c.symbol || '').slice(0, 3)}</div>
              ...
              <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setMetaForm(null)}>Cancelar</button><button className="t-btn" onClick={saveMeta}>Salvar meta</button></div>
            </div></div>
          </div>
          )
        })()}
          ...
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setUpgrade(null) }}>
            <div className="sheet"><div className="grabber" />
              ...
              <div className="grid2" style={{ marginTop: 4 }}>
                <button className="t-btn ghost" onClick={() => setUpgrade(null)}>Agora não</button>
                <button className="t-btn" onClick={() => router.push('/assinar')}>Fazer upgrade</button>
              </div>
            </div>
          </div>
        )}
              ...
              <div className="sheet"><div className="grabber" />
                <div className="sheet-scroll">
                  ...
                  {h.kind === 'crypto' && h.cg_id && !L?.usd && (
                    <div className="t-card" style={{ marginTop: 10, background: 'rgba(255,90,90,.10)', border: '1px solid rgba(255,90,90,.35)' }}>
                  ...
                  {h.kind === 'crypto' && (() => {
                    ...
                    return (<div className="t-card" style={{ marginTop: 14 }}>
                    ...
                    <div className="lock-card" style={{ marginTop: 14 }}>
                      ...
                      <button className="t-btn" onClick={() => { setDetail(null); router.push('/assinar') }}>Fazer upgrade para PRO</button>
                    </div>
                  )}
                  ...
                  {h.kind === 'crypto' && aporteTotal > 0 && (
                    <div className="t-card" style={{ marginTop: 14, border: jaProtegido ? '1px solid rgba(74,222,128,.45)' : '1px solid var(--line)', background: jaProtegido ? 'rgba(74,222,128,.08)' : undefined }}>
                      ...
                      {!jaProtegido && qtyRecuperar > 0 && (
                        <button className="t-btn" style={{ width: '100%', marginTop: 10 }} onClick={() => setMoveForm({ symbol: h.symbol, name: h.name, cg_id: h.cg_id, color: h.color, dir: 'sell', qty: fmt(qtyRecuperar, qtyRecuperar < 1 ? 6 : 3).replace('.', ','), price: String(precoVivo), note: 'recuperação de capital', buy_date: new Date().toISOString().slice(0, 10), toCash: true, maxQty: h.qty })}>
                  ...
                  <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => openMove(h, 'sell')}>↘ Vender</button><button className="t-btn" onClick={() => openBuy(h)}>+ Registrar compra</button></div><div className="grid2" style={{ marginTop: 8 }}><button className="t-btn ghost" onClick={() => openMove(h, 'to_pool')}>↗ Saída p/ pool</button><button className="t-btn ghost" onClick={() => openMove(h, 'from_pool')}>↙ Retorno de pool</button></div><div style={{ marginTop: 8 }}><button className="t-btn ghost danger" style={{ width: '100%' }} onClick={() => delAsset(h)}>Excluir ativo</button></div>
                </div>
              </div>
            </div>
          )
        ...
        {moveForm && (
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setMoveForm(null) }}>
            <div className="sheet"><div className="grabber" /><div className="sheet-scroll">
              ...
              <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setMoveForm(null)}>Cancelar</button><button className="t-btn" onClick={saveMove}>{moveForm.dir === 'sell' ? 'Confirmar venda' : 'Salvar movimento'}</button></div>
            </div></div>
          </div>
        )}
        ...
        {txEdit && (() => {
          ...
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setTxEdit(null) }}>
            <div className="sheet"><div className="grabber" /><div className="sheet-scroll">
              ...
              <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setTxEdit(null)}>Cancelar</button><button className="t-btn" onClick={saveTxEdit}>Salvar correção</button></div>
            </div></div>
          </div>
          )
        })()}
            ...
            <div className="sheet"><div className="grabber" /><div className="sheet-scroll">
              ...
              {txForm.isNew && (<>
                ...
                {coinResults && coinResults.length > 0 && (
                  <div className="t-card" style={{ padding: 6, marginTop: 6 }}>
                    {coinResults.map((c: any) => (
                      <div key={c.id} onClick={() => pickCoin(c)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px', cursor: 'pointer', borderRadius: 8 }}>
                        {c.image ? <img src={c.image} alt="" width={24} height={24} style={{ borderRadius: '50%' }} /> : <span className="sym" style={{ width: 24, height: 24, background: 'linear-gradient(145deg,#19D3A2,#19D3A288)' }}>{c.symbol.slice(0, 3)}</span>}
                ...
                {txForm.cg_id && !coinManual && (
                  <div className="t-card" style={{ marginTop: 8, background: 'rgba(43,255,198,.08)', border: '1px solid rgba(43,255,198,.3)' }}>
              ...
              {txForm.cg_id && (
                <div className="field">
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderRadius: 12, border: '1px solid rgba(43,255,198,.28)', background: 'rgba(43,255,198,.06)' }}>
                    {txForm.img ? <img src={txForm.img} alt="" width={26} height={26} style={{ borderRadius: '50%' }} /> : <span className="sym" style={{ width: 26, height: 26, background: `linear-gradient(145deg,${txForm.color || '#19D3A2'},${(txForm.color || '#19D3A2')}88)` }}>{(txForm.symbol || '?').slice(0, 3)}</span>}
              ...
              <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setTxForm(null)}>Cancelar</button><button className="t-btn" onClick={saveBuy}>Salvar compra</button></div>
            </div></div>
          </div>
        )}
        ...
        {poolForm && (
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setPoolForm(null) }}>
            <div className="sheet"><div className="grabber" /><div className="sheet-scroll">
              ...
              <div className="grid2"><div className="field"><label>Rede on-chain (base, bsc, arbitrum, ethereum)</label><input value={poolForm.network || 'base'} onChange={e => setPoolForm({ ...poolForm, network: e.target.value.trim().toLowerCase() })} placeholder="base" /></div><div className="field"><label>Endereço da pool (tração)</label><input value={poolForm.pool_address || ''} onChange={e => setPoolForm({ ...poolForm, pool_address: e.target.value })} placeholder="0x... (opcional)" /></div></div>
              ...
              <div className="grid2" style={{ marginTop: 16 }}>{poolForm.id && <button className="t-btn ghost danger" onClick={() => delPool(poolForm.id)}>Excluir</button>}<button className="t-btn ghost" onClick={() => setPoolForm(null)}>Cancelar</button><button className="t-btn" onClick={savePool}>Salvar</button></div>
            </div></div>
          </div>
        )}
          ...
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setPerpForm(null) }}>
            <div className="sheet"><div className="grabber" /><div className="sheet-scroll">
              ...
              {!perpForm.id && perpPick && filtered.length > 0 && (
                <div className="t-card" style={{ padding: 6, marginTop: 6, maxHeight: 220, overflowY: 'auto' }}>
                  {filtered.slice(0, 24).map(m => (
                    <div key={m.symbol} onClick={() => pickPerpMkt(m)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 8, cursor: 'pointer', borderRadius: 8 }}>
                      <div style={{ flex: 1, minWidth: 0 }}><b style={{ fontSize: 13 }}>{m.symbol}</b> <span style={{ color: 'var(--muted)', fontSize: 11 }}>{m.name}</span> <span className="t-chip" style={{ background: 'rgba(25,211,162,.15)', color: 'var(--purple)' }}>{m.maxLev}x</span></div>
              ...
              {perpForm.symbol && (
                <div className="t-card" style={{ marginTop: 8, background: 'rgba(25,211,162,.08)', border: '1px solid rgba(25,211,162,.3)' }}>
              ...
              <div className="field"><label>Direção</label>
                <div className="segbar">
                  {(['long', 'short'] as PerpSide[]).map(s => (
                    <button key={s} className={perpForm.side === s ? 'seg on' : 'seg'} onClick={() => setPerpForm({ ...perpForm, side: s })} style={perpForm.side === s ? { background: s === 'long' ? 'linear-gradient(135deg,#0f9d63,#22EE80)' : 'linear-gradient(135deg,#B21548,var(--red))' } : undefined}>{s === 'long' ? '▲ Long' : '▼ Short'}</button>
                  ))}
                </div>
              </div>
              ...
              <a className="t-btn" style={{ textDecoration: 'none', textAlign: 'center', marginTop: 14, background: 'linear-gradient(135deg,#0B7A55,#19D3A2)' }} href="https://app.ondoperps.xyz/" target="_blank" rel="noreferrer">Abrir na Ondo Perps ↗</a>
              <div className="grid2" style={{ marginTop: 10 }}>
                {perpForm.id && <button className="t-btn ghost danger" onClick={() => delPerp(perpForm.id)}>Excluir</button>}
                <button className="t-btn ghost" onClick={() => setPerpForm(null)}>Cancelar</button>
                <button className="t-btn" onClick={savePerp}>{perpForm.id ? 'Salvar' : 'Registrar posição'}</button>
              </div>
            </div></div>
          </div>
          )
        ...
        {perpClose && (() => {
          ...
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setPerpClose(null) }}>
            <div className="sheet"><div className="grabber" /><div className="sheet-scroll">
              ...
              <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setPerpClose(null)}>Cancelar</button><button className="t-btn" onClick={closePerp}>Confirmar fechamento</button></div>
            </div></div>
          </div>
          )
        })()}
        ...
        {perpAcctForm !== null && (
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setPerpAcctForm(null) }}>
            <div className="sheet"><div className="grabber" /><div className="sheet-scroll">
              ...
              <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setPerpAcctForm(null)}>Cancelar</button><button className="t-btn" onClick={savePerpCollateral}>Salvar</button></div>
            </div></div>
          </div>
        )}
            ...
            <div className="modal" onClick={e => { if (e.target === e.currentTarget && !importer.busy) setImporter(null) }}>
              <div className="sheet sheet-scroll"><div className="grabber" />
                ...
                <div className="field" style={{ marginTop: 12 }}>
                  ...
                  <textarea value={importer.text} onChange={e => setImporter({ ...importer, text: e.target.value })} placeholder={"15/01/2021\t-\t21.500,00\n06/08/2021\t2.000,00\t-"} style={{ width: '100%', minHeight: 130, fontFamily: "'JetBrains Mono', ui-monospace, monospace", fontSize: 13, background: 'rgba(6,14,10,.65)', border: '1px solid var(--line-strong)', color: 'var(--text)', borderRadius: 11, padding: 12, resize: 'vertical' }} />
                </div>
                ...
                <div style={{ marginTop: 16, display: 'flex', gap: 9 }}>
                  <button className="t-btn" disabled={!rows.length || importer.busy} onClick={runImport}>{importer.busy ? 'Importando…' : `Importar ${rows.length}`}</button>
                  <button className="t-btn ghost" disabled={importer.busy} onClick={() => setImporter(null)}>Cancelar</button>
                </div>
              </div>
            </div>
          )
          ...
          return (
            <div className="modal" onClick={e => { if (e.target === e.currentTarget) setCalc(null) }}>
              <div className="sheet sheet-scroll"><div className="grabber" />
                ...
                <button className="t-btn ghost" style={{ marginTop: 12 }} onClick={() => setCalc(null)}>Fechar</button>
              </div>
            </div>
          )
        })()}
        ...
        {flowForm && (
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setFlowForm(null) }}>
            <div className="sheet"><div className="grabber" />
              ...
              <div className="grid2" style={{ marginTop: 16 }}>{flowForm.id && <button className="t-btn ghost danger" onClick={() => delFlow(flowForm.id)}>Excluir</button>}<button className="t-btn ghost" onClick={() => setFlowForm(null)}>Cancelar</button><button className="t-btn" onClick={saveFlow}>Salvar</button></div>
            </div>
          </div>
        )}
        ...
        {radarDetail && (
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setRadarDetail(null) }}>
            <div className="sheet"><div className="grabber" />
              <div className="sheet-scroll">
                <h3><span className="qsym" style={{ width: 32, height: 32 }}>{radarDetail.image ? <img src={radarDetail.image} alt="" /> : (radarDetail.symbol || '?').slice(0, 3)}</span>{radarDetail.name}<span style={{ marginLeft: 'auto', fontFamily: "'JetBrains Mono', ui-monospace, monospace", fontSize: 14 }}>{usd(radarDetail.price)}</span></h3>
                ...
                <div style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setRadarDetail(null)}>Fechar</button></div>
              </div>
            </div>
          </div>
        )}
              ...
              <div className="sheet-scroll">
                ...
                {allAlerts.slice().sort((a, b) => (seenAlerts.includes(a.id) ? 1 : 0) - (seenAlerts.includes(b.id) ? 1 : 0)).map(a => (
                  <div className={`alert-item alert-${a.tone}`} key={a.id} style={seenAlerts.includes(a.id) ? { opacity: .6 } : undefined}>
                    ...
                    <div className="alert-t"><b>{a.title}{!seenAlerts.includes(a.id) && <span style={{ marginLeft: 8, fontSize: 9, color: 'var(--pink-bright)', fontFamily: "'JetBrains Mono', ui-monospace, monospace", verticalAlign: 'middle' }}>● NOVO</span>}</b><span>{a.text}</span></div>
                  </div>
                ))}
                <div style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setAlertsOpen(false)}>Fechar</button></div>
              </div>
            </div>
          </div>
        )}
        ...
        {levelForm && (
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setLevelForm(null) }}>
            <div className="sheet"><div className="grabber" />
              ...
              <div className="grid2" style={{ marginTop: 16 }}>{levelForm.id && <button className="t-btn ghost danger" onClick={() => delLevel(levelForm.id)}>Excluir</button>}<button className="t-btn ghost" onClick={() => setLevelForm(null)}>Cancelar</button><button className="t-btn" onClick={saveLevel}>Salvar</button></div>
            </div>
          </div>
        )}
        ...
        {editDraft && (
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setEditDraft(null) }}>
            <div className="sheet"><div className="grabber" />
              ...
              <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setEditDraft(null)}>Cancelar</button><button className="t-btn" onClick={saveEdit}>Salvar</button></div>
            </div>
          </div>
        )}
        ...
        {assetEdit && (
          <div className="modal" onClick={e => { if (e.target === e.currentTarget) setAssetEdit(null) }}>
            <div className="sheet"><div className="grabber" />
              ...
              <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setAssetEdit(null)}>Cancelar</button><button className="t-btn" onClick={saveAssetEdit}>Salvar</button></div>
            </div>
          </div>
        )}
      </div>
