...
const CHAIN_COLOR: Record<string, string> = { Ethereum: '#40BEFF', Solana: '#2BFFC6', Base: '#3C7DFF', Arbitrum: '#3AA0FF', BSC: '#F5A623', Polygon: '#19D3A2' }
...
export default function MonitorTiger() {
  ...
  if (err) return <div className="t-card section-gap"><div className="eyebrow">Monitor Tiger</div><p className="foot-note" style={{ textAlign: 'left', padding: 0, marginTop: 8 }}>Não foi possível carregar os dados on-chain agora. Tente novamente em instantes.</p></div>
  ...
  return (
    <>
      ...
      {r && (
        <div className="t-card section-gap">
          <div className="mon-h"><span className="eyebrow" style={{ margin: 0 }}>Regime de mercado · dominância</span>{regimeRead && <b style={{ color: regimeRead.c, fontFamily: "'Space Grotesk', Inter, sans-serif", fontSize: 12 }}>{regimeRead.t}</b>}</div>
          <div className="mon-dombar">
            <div style={{ width: `${r.btc}%`, background: '#F5A623' }} title="BTC" />
            <div style={{ width: `${r.eth}%`, background: '#40BEFF' }} title="ETH" />
          ...
          <div className="mon-domlbl">
            ...
            <span><i style={{ background: '#40BEFF' }} />ETH {r.eth.toFixed(1)}%</span>
      ...
      {d.stables && (
        <div className="t-card section-gap">
          <div className="mon-h"><span className="eyebrow" style={{ margin: 0 }}>Pólvora seca · oferta de stablecoins</span><b className={d.stables.d30 >= 0 ? 'up' : 'down'} style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>{pct(d.stables.d30)} 30d</b></div>
      ...
      {d.fng && (() => {
        ...
        return (
          <div className="t-card section-gap">
            <div className="mon-h"><span className="eyebrow" style={{ margin: 0 }}>Medo & Ganância</span><b style={{ color: col, fontFamily: "'Space Grotesk', Inter, sans-serif" }}>{d.fng.label}</b></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 8 }}>
              <div style={{ fontFamily: "'Space Grotesk', Inter, sans-serif", fontWeight: 800, fontSize: 40, color: col }}>{v}</div>
              <div style={{ flex: 1 }}>
