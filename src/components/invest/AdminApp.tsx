...
import type { AdminData, AdminUser } from '@/lib/invest/admin-data'
import type { Plan } from '@/lib/invest/plans'
import { adminRefresh, setUserPlan, cancelUserSub, savePlan, ajustarCredito, setTaxaMp } from '@/app/admin/assinaturas/actions'

const brl = (cents: number) => 'R$ ' + (cents / 100).toFixed(2).replace('.', ',')
const dt = (s: string | null) => s ? new Date(s).toLocaleDateString('pt-BR') : '—'
const PLAN_IDS = ['start', 'pro', 'alpha']
const planColor: Record<string, string> = { start: '#40BEFF', pro: '#00FF88', alpha: '#FFB020' }
  ...
  return (
    <div className="adm">
      <header className="adm-top">
        <div className="adm-brand"><LogoMark size={26} /> Tiger Labs <b>· Admin</b></div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="adm-gbtn" onClick={() => router.push('/invest')}>App</button>
      ...
      <main className="adm-main">
        {tab === 'visao' && (
          <div className="adm-cards">
            ...
            <Card label="Receita total" value={brl(m.revenueCents)} accent="#00FF88" />
              ...
              <div style={{ display: 'flex', gap: 18, marginTop: 10, flexWrap: 'wrap' }}>
                {PLAN_IDS.map(pid => (
                  <div key={pid} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <b style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>{m.byPlan[pid] || 0}</b>
            ...
            {data.orders.map(o => (
              <div key={o.id} className="adm-orow">
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace", fontWeight: 700 }}>{brl(o.amountCents)}</div>
          ...
          {r.indicadores.map(i => (
            <div key={i.userId} className="adm-orow">
              ...
              <div style={{ textAlign: 'right', display: 'flex', gap: 12, alignItems: 'center' }}>
                <div><div style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace", fontWeight: 700, color: '#2BFFC6' }}>{brl2(i.saldoCents)}</div><div className="adm-usub">ganho {brl2(i.ganhoCents)}</div></div>
        ...
        <div className="adm-list">
          ...
          {r.transacoes.map(t => (
            <div key={t.id} className="adm-orow">
              ...
              <b style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace", color: t.valor_cents >= 0 ? '#2BFFC6' : 'var(--red)' }}>{t.valor_cents >= 0 ? '+' : '-'}{brl2(Math.abs(t.valor_cents))}</b>
            </div>
          ))}
        </div>
      )}
      ...
      {aj && (
        <div className="modal" onClick={e => { if (e.target === e.currentTarget) setAj(null) }}>
          <div className="sheet"><div className="grabber" />
            ...
            <div className="grid2" style={{ marginTop: 16 }}><button className="t-btn ghost" onClick={() => setAj(null)}>Cancelar</button><button className="t-btn" disabled={busy} onClick={salvarAjuste}>Salvar ajuste</button></div>
          </div>
        </div>
      )}
    </div>
