...
export default function ReferralApp({ linkBase }: { linkBase: string }) {
  ...
  async function pagar(planId: string, cycle: string) {
    ...
    setBusy('')
    if (r.ok) { setMsg('✓ Assinatura ativada com seus créditos!'); await load(); setTimeout(() => router.push('/invest'), 1400) }
    else setMsg('✗ ' + r.erro)
  }
    ...
    <>
      ...
      <main className="ref-wrap">
        <div className="ref-top">
          <div className="ref-brand"><LogoMark size={26} /> Tiger Labs <b>· Indicações</b></div>
          <button className="adm-gbtn" onClick={() => router.push('/invest')}>Voltar ao app</button>
        </div>
