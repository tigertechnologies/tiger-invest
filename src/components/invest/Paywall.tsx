...
import Background from '@/components/invest/Background'
import { createClient } from '@/lib/invest/sb-client'
import { PLANS, type Plan } from '@/lib/invest/plans'
...
export default function Paywall({ userEmail }: { userEmail: string }) {
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      if ('status' in s && s.status === 'paid') { setPaid(true); clearInterval(pollRef.current); setTimeout(() => router.push('/invest'), 1500) }
    }, 4000)
      ...
      <main className="as-wrap">
        <div className="as-card" style={{ maxWidth: 460 }}>
          {!pix && (<>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 13, marginBottom: 14 }}>Escolha seu plano para liberar o Tiger Labs. Pagamento via PIX, cancele quando quiser.</p>
            ...
            <button className="t-btn" style={{ marginTop: 16 }} onClick={gerarPix} disabled={pixLoading}>{pixLoading ? 'Gerando PIX…' : `Assinar ${plan.name.replace('TIGER ', '')} · R$ ${price.toFixed(2).replace('.', ',')}`}</button>
          ...
          {pix && (<>
            ...
            {!paid && pix.ok && (
              <div style={{ textAlign: 'center', marginTop: 6 }}>
                ...
                <div style={{ margin: '16px 0 6px', fontFamily: "'JetBrains Mono', ui-monospace, monospace", fontSize: 11, color: 'var(--muted)', wordBreak: 'break-all', background: 'rgba(255,255,255,.05)', border: '1px solid var(--line)', borderRadius: 10, padding: '10px 12px', maxHeight: 84, overflow: 'auto' }}>{pix.qrCode}</div>
                <button className="t-btn" onClick={copyCode} style={{ marginTop: 8 }}>{copied ? 'Copiado ✓' : 'Copiar código PIX'}</button>
                <p style={{ fontSize: 12, color: 'var(--faint)', marginTop: 14 }}>Aguardando pagamento…</p>
              </div>
            )}
            {!paid && !pix.ok && (<><p className="as-msg" style={{ color: 'var(--red)' }}>{pix.erro}</p><button className="t-btn" onClick={() => setPix(null)} style={{ marginTop: 8 }}>Voltar</button></>)}
          </>)}
        </div>
      </main>
    </>
